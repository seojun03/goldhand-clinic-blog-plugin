...
description: 금손한의원 정보형 네이버 블로그 글을 만든다. 2024-10-04 이후 직접 본문까지 검토한 위석부부한의원 정보글 11편 중 최근 3개와 겹치지 않는 한 편에서 주제·독자 고민·핵심 일반 정보·정보 공개 순서만 가져오고, 말투는 2026-08-21 현재 금손 공식 블로그 공개 글 74편을 전수 분석한 `goldhand-official-voice-v1`로만 쓴다. 위석 말투·문장·업체 사실·경력·사례·성과 수치는 쓰지 않는다. 제목에는 본문이 실제로 답하는 개수 숫자를 사용하고, 도입에는 3분 읽기 안내와 핵심 하이라이트를 둔다. 시각 자료는 사용자 소유 callilife 작품을 참고한 GPT Image 1~3장과 플러그인 `assets/official-media`에 내장된 실제 금손 사진 6~12장을 함께 사용한다. 실제 사진은 원장이 환자를 치료·진찰·상담·검사하는 사람 대 사람 현장 사진만 쓰며 로고·건물·약·장비·빈 공간은 절대 쓰지 않는다. 이미지 아래 보이는 캡션은 쓰지 않는다. 최근 3개 글의 실제 사진은 먼저 제외하고 새 승인 사진이 6장보다 적을 때만 같은 승인 사진을 최소 수량까지 재사용한다. 모든 글 마지막에는 승인된 원장 상담 사진 자체에 금손 공식 블로그 링크를 걸고 네이버 플레이스 지도 블록을 고정 삽입한다. `ㅎㅎ`, `ㅠㅠ`, `^^`, 이모지뿐 아니라 실제 진료실에서 말하지 않을 AI 번역투·작문체·감성적 행동 유도 문장군과 추상 명사 중심의 압축 표현을 차단한다. 꾸밈은 중앙 정렬·네이버 순정 인용구·구분선·표1·고정 가치입증·50:50 운영정보의 `goldhand-naver-native-v4`로 고정하고 제목 키워드 1회·본문 2~3회, 1,400~1,800자, 모바일 2~3줄을 적용해 바탕화면에 복사용 HTML을 저장한다.
---
...
29. 모든 글의 `clinic-info` 운영정보 표가 끝난 뒤에는 `assets/goldhand-closing-links.json`의 고정 사진형 공식 블로그 링크와 네이버 플레이스 지도 블록을 정확히 한 번 붙인다. 지정된 승인 원장-환자 상담 사진 자체를 클릭하면 금손 공식 블로그 `https://blog.naver.com/goldhand7582_`로 이동해야 하며 제목·소개·`blog.naver.com` 같은 보이는 링크 문구나 OG 카드는 만들지 않는다. 지도는 네이버 플레이스 ID `1598180269`의 `https://map.naver.com/p/entry/place/1598180269`로 고정한다. 복사용 HTML에는 실제 `<a><img></a>` fallback, 네이버 순정 `se-image`·`v2_image`, `se-placesMap`·`v2_map` 구조와 두 개의 `data-module-v2`를 함께 남긴다. 이 묶음 뒤에는 다른 본문 컴포넌트를 두지 않는다.
...
- 고정 글말미 블로그 링크·네이버 지도: `assets/goldhand-closing-links.json`
...
13. `build_naver_copy_page.py`가 운영정보 뒤에 승인 상담 사진 자체를 누르는 금손 공식 블로그 링크와 네이버 플레이스 지도 블록을 한 번만 붙이게 한다. 복사 미리보기에서 `photoLinks=1`, `maps=1`, `nativeModules=2`와 보이는 링크 문구 0개를 확인한다.
...
8. **진료실 발화 가능성·생활어 검사**: 문장마다 `박준희 원장이 환자를 앞에 두고 실제로 이렇게 말할까?`, `환자가 듣자마자 어느 동작에서 어디가 어떻게 불편한지 떠올릴 수 있을까?`를 묻는다. 입으로 말하면 어색한 번역투·작문체·감성 문장과 추상 명사 중심의 압축 표현은 단어만 바꾸지 말고 문장 구조부터 짧고 직접적인 한국어 구어체로 다시 쓴다. 안전 경계는 `혼자 판단해서 운동을 계속하시면 안 됩니다`처럼 분명하게 말하고, 관찰 정보가 필요하면 `어떤 동작에서 다시 아픈지 진료할 때 말씀해 주세요`처럼 실제 진료 행동으로 연결한다.
9. **독립 검수**: 제목, 사실 팩, 선택 콘텐츠 브리프, 금손 말투 프로필, 초안만 새 패스에서 읽는다. 특정 금지 문구뿐 아니라 같은 생성 원리에서 나온 문장군을 찾는다.
10. **부분 수정**: 실패 문장과 필요한 앞뒤 문장만 고친다.
11. **SEO·모바일·이미지·순정 컴포넌트·HTML 후처리**: 자연스러운 글을 먼저 완성한 뒤 정확 키워드를 2~3회 넣고, 완성 문장의 표현을 바꾸지 않은 채 모바일 시각 줄로 나눈다. GPT Image 생성본 1~3장과 플러그인 내장 승인 원장-환자 실제 사진 6~12장을 관련 문단 뒤에 한 장씩 분산한다. 실제 사진은 새 승인 사진 우선이며, 6장 미만일 때만 최근 승인 사진을 최소 수량까지 재사용한다. 모든 글을 중앙 정렬하고 필요한 순정 컴포넌트를 배치한다.
...
- 고정 운영정보 누락, 글 마지막의 클릭 가능한 승인 상담 사진·네이버 플레이스 지도 블록 누락·중복·링크 오기·로고 사진 사용, 사진 대신 제목·소개·URL이 보이는 OG 링크 카드 출력, 또는 제작 지시·출처 목록 노출

## 완료 보고

최종 응답에는 제목, `콘텐츠 레퍼런스` 링크 한 편, 금손 공식 말투·생활어 검수, 최근 3개 주제·실제 사진 중복 검사, 원장-환자 실제 사진 수와 최근 승인 사진 재사용 수, 로고·건물·약·장비·빈 공간 사진 0장, GPT Image 수, 공백 제외 글자 수, 제목·본문 키워드 횟수, 내용 순서·복사 거리, 전 문단 중앙 정렬, 노란 하이라이트·밑줄·주의용 빨간 글씨, 고정 가치입증 6행, 50:50 운영정보 표, 글말미 클릭형 공식 블로그 상담 사진·네이버 지도, 모바일 2~3줄 문단·네이버 순정 컴포넌트, 의료·사실·HTML 검사 결과와 저장 경로를 간단히 적는다.

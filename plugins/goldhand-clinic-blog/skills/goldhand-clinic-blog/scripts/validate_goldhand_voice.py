...
class ProseParser(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        if tag in {"table", "script", "style"}:
            self.skip_depth += 1
        elif tag in {"p", "section", "blockquote", "h2", "h3", "br", "hr"} and not self.skip_depth:
            self.parts.append("\n")
...
def validate(fragment: str, profile: dict[str, object]) -> dict[str, object]:
    ...
    if concrete < int(required.get("minimumConcreteDailyScenes", 2)):
        error("concrete-scene-dropout", "추상 설명보다 실제 생활 동작과 몸의 장면을 두 가지 이상 넣으세요.")
    if abstract >= 18 and abstract > concrete:

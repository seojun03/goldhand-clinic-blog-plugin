...
- 네이버 내부 `se-*` 클래스, 레퍼런스 업체의 로고·사진·지도·연락처를 복사하지 않음
...
선택 우선순위와 최근 3개 fallback:

1. `safeAuto: true`이며 `personInteraction: true`인 원장-환자 치료 장면
2. `safeAuto: true`이며 `personInteraction: true`인 원장-환자 진찰·상담·설명 장면
3. 같은 글과 최근 3개 글에서 사용하지 않은 ID·파일 해시
4. 그 밖의 주제 일치 치료 사진
5. 사람 중심 장면만으로 수량이 부족할 때 치료 재료·한약·장비·원내 공간
6. 새 안전 사진이 6장 미만일 때만 최근 3개 글의 안전한 사람 중심 신뢰 사진부터 최소 6장까지 재사용

새 안전 사진이 이미 6장 이상이면 최근 사진으로 선호 수량을 더 채우지 않는다. 최근 신뢰 사진까지 써도 6장이 안 되면 발행을 중단한다.

...
10. 연락처는 레퍼런스에 따라 바꾸지 않는다. `진료시간 안내` 제목 아래 `clinic-hours` 3열 표를 `24% / 38% / 38%` 폭으로 두고, 이어서 금손 골드 제목띠와 위치·찾아오는 길·전화·카카오톡·네이버 예약을 한 행씩 쌓은 `clinic-info` 1열 표를 빠짐없이 한 번만 둔다.

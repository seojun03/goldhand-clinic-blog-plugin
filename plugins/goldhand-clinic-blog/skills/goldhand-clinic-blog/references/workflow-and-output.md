...
`scripts/select_wipark_content_reference.py`는 본문 검토가 끝난 `wipark-content-briefs.json`의 11편만 사용한다. 최근 3개와 같은 레퍼런스와 핵심 주제는 제외하고, 선택한 한 편만 **주제·독자 고민·핵심 일반 정보·정보 공개 순서**를 통제한다. `sourceToneBlocked=true`와 `voiceProfileId=goldhand-official-voice-v1`이 아니면 쓰지 않는다. 위석 말투는 통제권이 없고 꾸밈은 `goldhand-naver-native-v4`이다.
...
4. 선택한 위석 콘텐츠 브리프 한 편의 주제·독자 고민·필수 일반 정보 순서
...
2. 검수 완료 본문을 `<article data-goldhand-type="정보전달형" data-editorial-mode="content-reference-goldhand-voice" data-editorial-master-id="..." data-content-reference-source="..." data-editorial-reference-source="..." data-editorial-source-role="topic-reader-concerns-general-information-sequence-only" data-goldhand-voice-profile="goldhand-official-voice-v1" data-goldhand-design-system="goldhand-naver-native-v4">` 하나로 만든다.
...
7. 실제 행·열 정보에는 `data-naver-native-component="table" data-native-table-preset="naver-table1-default"`를 둔다. `credential`·`clinic-info`는 각각 정확히 한 번 사용하고 `article-summary`는 실제 비교 정보가 있을 때만 한 번 사용한다. 모든 셀에 회색 구분선과 가로·세로 중앙 정렬을 적용한다.

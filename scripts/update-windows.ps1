...
$Repository = "seojun03/goldhand-clinic-blog-plugin"
$ReleaseAssetName = "goldhand-clinic-blog-plugin.zip"
$StatePath = Join-Path $EditableRoot ".goldhand-managed-release"
$mutex = New-Object Threading.Mutex($false, "Local\GoldhandClinicPluginUpdate")
$hasLock = $false

function Write-Update([string]$Message) {
    Write-Host "[Goldhand Clinic Blog updater] $Message" -ForegroundColor Cyan
}
...
function Get-LocalArchiveRelease {
    param([string]$ArchivePath)
    $resolved = (Resolve-Path -LiteralPath $ArchivePath).Path
    $hash = (Get-FileHash -LiteralPath $resolved -Algorithm SHA256).Hash.ToLowerInvariant()
    $tag = if ($env:GOLDHAND_RELEASE_TAG) { [string]$env:GOLDHAND_RELEASE_TAG } else { "local-$hash" }
    return [PSCustomObject]@{ Tag = $tag; Archive = $resolved; DownloadUrl = $null }
}

function Get-LatestRelease {
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $headers = @{ "User-Agent" = "GoldhandClinicPluginUpdater"; "Accept" = "application/vnd.github+json" }
    ...
    if (-not $asset) {
        throw "The latest validated release does not contain $ReleaseAssetName."
    }
...
try {
    ...
    if (-not $hasLock) {
        Write-Update "Another update is already running."
        return
    }

    $release = if ($env:GOLDHAND_UPDATE_ARCHIVE) {
        Get-LocalArchiveRelease -ArchivePath $env:GOLDHAND_UPDATE_ARCHIVE
    } else {
        Get-LatestRelease
    }
    $current = if (Test-Path -LiteralPath $StatePath -PathType Leaf) {
        (Get-Content -LiteralPath $StatePath -Raw -Encoding UTF8).Trim()
    } else {
        ""
    }
    if ($env:GOLDHAND_FORCE_UPDATE -ne "1" -and $current -eq $release.Tag) {
        Write-Update "Already current: $current"
        return
    }

    $tempBase = [Environment]::GetFolderPath("UserProfile")
    if ([string]::IsNullOrWhiteSpace($tempBase)) { $tempBase = [IO.Path]::GetTempPath() }
    $tempRoot = Join-Path $tempBase (".ghu-" + [Guid]::NewGuid().ToString("N").Substring(0, 8))
    try {
        New-Item -ItemType Directory -Path $tempRoot -Force | Out-Null
        $archive = Join-Path $tempRoot "release.zip"
        if ($release.Archive) {
            Copy-Item -LiteralPath $release.Archive -Destination $archive -Force
        } else {
            Invoke-WebRequest -UseBasicParsing -Uri $release.DownloadUrl -OutFile $archive
        }
        ...
        if (-not $installer) {
            throw "The validated release ZIP is missing install-from-download-windows.ps1."
        }

        $previousTag = $env:GOLDHAND_RELEASE_TAG
        ...
        try {
            ...
            if ($null -eq $previousSkip) { Remove-Item Env:GOLDHAND_SKIP_AUTO_UPDATE_REGISTRATION -ErrorAction SilentlyContinue } else { $env:GOLDHAND_SKIP_AUTO_UPDATE_REGISTRATION = $previousSkip }
        }
        $installedTag = if (Test-Path -LiteralPath $StatePath -PathType Leaf) {
            (Get-Content -LiteralPath $StatePath -Raw -Encoding UTF8).Trim()
        } else {
            ""
        }
        if ($installedTag -ne $release.Tag) {
            throw "The managed release state was not updated to $($release.Tag)."
        }
        Write-Update "UPDATED TO $($release.Tag)"
    } finally {
        Remove-TempDirectoryBestEffort -LiteralPath $tempRoot
    }
} catch {
